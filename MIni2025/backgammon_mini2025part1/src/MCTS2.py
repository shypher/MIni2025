...
class MCTS_shayOren2(Strategy):
    ...
    def move(self, board, colour, dice_roll, make_move, opponents_activity):
        log("Starting MCTS2 move calculation")
...
class MCTS_shayOren3(Strategy):
    ...
    def move(self, board, colour, dice_roll, make_move, opponents_activity):
        log("Starting MCTS3 move calculation")
...
class MCTS_shayOren5(Strategy):
    ...
    def move(self, board, colour, dice_roll, make_move, opponents_activity):
        ...
        start_time = time.time()
        time_limit = board.getTheTimeLim() - 0.85 if board.getTheTimeLim() != -1 else float('inf')
        self.tree = MCTSNode(state=board, colour=colour, dice_roll=dice_roll)
        iterations = 0
        root = self.tree
    ...
    def treePolicy(self, node, c=1):
        if node is not None:
            while not node.state.has_game_ended():
                if not node.is_fully_expanded():
                    return node.expand()
                else:
                    node = node.best_child(exploration_weight=c)
            return node            
