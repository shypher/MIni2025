...
import random
from backgammon_mini2025part1.src.compare_all_moves_strategy import CompareAllMovesWeightingDistanceAndSinglesWithEndGame
...
class RL_player_old(Strategy):
    ...
    def move(self, board, color, dice_roll, make_move, opponents_activity):
        ...
        for simulated_board, move_seq in candidate_moves.items():
            ...
            move_scores.append((output, move_seq,simulated_board ))        
        move_scores.sort(reverse=True, key=lambda x: x[0])
        top_moves = move_scores[:1]
        
        if top_moves:
            scores = [np.exp(score) for score, _, _ in top_moves]
            total_score = sum(scores)
            probabilities = [score / total_score for score in scores]
            best_move_sequence = random.choices([move_seq for _, move_seq, _ in top_moves], weights=probabilities, k=1)[0]
